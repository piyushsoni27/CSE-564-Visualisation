...
function plotStats(data) {
    ...
    plotOuter.append("text").attr("x", 140).attr("y", 95).text("Avgerage Daily Cases").attr("fill", "#ccc").attr("stroke", "#ccc").attr("text-anchor", "middle")
        .style("font-size", "20px");
    cases_avg = Math.floor(d3.mean(data, function(d) { return +d['new_cases']; }))
    plotOuter.append("text").attr("class", "text " + "new_cases").attr("x", 140).attr("y", 130).text(cases_avg).attr("text-anchor", "middle")
        .style("font-size", "40px").style("fill", "#FF884D");

    plotOuter.append("text").attr("x", 140).attr("y", 170).text("Avgerage Daily Deaths").attr("fill", "#ccc").attr("stroke", "#ccc").attr("text-anchor", "middle")
        .style("font-size", "20px");
    deaths_avg = Math.floor(d3.mean(data, function(d) { return +d['new_deaths']; }))
    plotOuter.append("text").attr("class", "text " + "new_deaths").attr("x", 140).attr("y", 205).text(deaths_avg).attr("text-anchor", "middle")
        .style("font-size", "35px").style("fill", "#FF884D");

    plotOuter.append("text").attr("x", 140).attr("y", 245).text("Avgerage Daily Vaccinations").attr("fill", "#ccc").attr("stroke", "#ccc").attr("text-anchor", "middle")
        .style("font-size", "20px");
    vaccinations_avg = Math.floor(d3.mean(data, function(d) { return +d['new_vaccinations']; }))
    plotOuter.append("text").attr("class", "text " + "new_vaccinations").attr("x", 140).attr("y", 280).text(vaccinations_avg).attr("text-anchor", "middle")
        .style("font-size", "35px").style("fill", "#FF884D");
